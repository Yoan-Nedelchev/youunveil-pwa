/**
 * Rider–Waite–Smith tarot faces (Pamela Colman Smith), public domain.
 * Resolved from Wikimedia Commons — Category:Rider-Waite-Smith_tarot_deck_(TaionWC).
 * Order here: API list order (Cups, Pentacles, Majors, Swords, Wands) before canonicalization.
 */

const RESOLVED_COMMONS_ORDER = [
...
const CUPS = RESOLVED_COMMONS_ORDER.slice(0, 14);
const PENTS = RESOLVED_COMMONS_ORDER.slice(14, 28);
const MAJORS = RESOLVED_COMMONS_ORDER.slice(28, 50);
const SWORDS = RESOLVED_COMMONS_ORDER.slice(50, 64);
const WANDS = RESOLVED_COMMONS_ORDER.slice(64, 78);

/** Canonical 78-card order: Major Arcana, then Wands, Cups, Swords, Pentacles (Ace→King each suit). */
export const RWS_CARD_FACE_URLS: readonly string[] = [
  ...MAJORS,
  ...WANDS,
  ...CUPS,
  ...SWORDS,
...
export const RWS_DECK_SIZE = 78;

export function getRwsCardFaceUrl(cardId: number): string {
  const u = RWS_CARD_FACE_URLS[cardId];
  if (!u) {
